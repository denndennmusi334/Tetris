...
void MinoManager::Initialize()
{
	...
	nextMino->SetGridPosition(Vec2i{ Config::FIELD_WIDTH + 2, 2 }); //ÉtÉBÅ[ÉãÉhÇÃâEë§Ç…éüÇÃÉ~ÉmÇï\é¶Ç∑ÇÈÇΩÇﬂÇÃà íu.
...
static constexpr Vec2i JLSTZ_KICK[4][4][5] =
{
	// UP (0)
	{
		...
		// UP -> RIGHT (0 -> 1)
		{
			{0,0},
			{-1,0},
			{-1,-1}, // å≥ 1 Å® èCê≥ -1 (è„Ç÷ÉLÉbÉN)
			{0,2},   // å≥ -2 Å® èCê≥ 2 (â∫Ç÷ÉLÉbÉN)
			{-1,2},  // å≥ -2 Å® èCê≥ 2
		},
...
void MinoManager::TryRotateRight()
{
	...
	for (int i = 0; i < 5; i++)
	{
		...
		if (IsMoveValid(testPos))
		{
			...
			lastKickIndex = i; // ç≈å„Ç…ê¨å˜ÇµÇΩÉLÉbÉNÇÃî‘çÜÇï€ë∂
			return;
		}
	}
